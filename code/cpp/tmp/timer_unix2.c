...
void set_time(int expireMS,int intervalMS)
{
    struct itimerspec       its;
    its.it_value.tv_sec = expireMS / 1000;
    its.it_value.tv_nsec = (expireMS % 1000)*1000000;
...
{
  printf("\npress ctl-c to quit.\n");
  if(makeTimer("First Timer",var, 0))
  {
    printf("\n timer error\n");
    return(1);
  }
  ...
  while(1)
  {
    ...
    {
      //struct itimerspec       its;
      //getitimer(firstTimerID,&its);
      //int val = its.it_value.tv_sec * 1000 + its.it_value.tv_nsec/1000000;
      //printf("\n existing va is %d",val);
      set_time(var,0);
    }
    else
    {
